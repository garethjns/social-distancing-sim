...
class TestPopulation(unittest.TestCase):
    ...
    def test_population_run(self):
        graph = Graph(community_n=15, community_size_mean=5)

        pop = Environment(
            name=f"{self._temp_dir.name}/example environment",
            disease=Disease(name="COVID-19"),
            healthcare=Healthcare(),
            observation_space=ObservationSpace(graph=graph, test_rate=1),
            environment_plotting=EnvironmentPlotting(
                ts_fields_g2=["Score"], ts_obs_fields_g2=["Observed score"]
            ),
        )

        pop.run(steps=50, plot=False, save=False)
    ...
    def test_population_run_with_testing_rate_lt_1(self):
        ...
        pop = Environment(
            ...
            disease=Disease(name="COVID-19"),
            healthcare=Healthcare(),
            observation_space=ObservationSpace(graph=graph, test_rate=0.2),
            environment_plotting=EnvironmentPlotting(
                ts_fields_g2=["Score"], ts_obs_fields_g2=["Observed score"]
            ),
        )
    ...
    def test_compare_two_pops(self):
        ...
        pop_close = Environment(
            name=f"{self._temp_dir.name}A herd of cats, observed",
            disease=disease,
            healthcare=healthcare,
            observation_space=ObservationSpace(
                graph=Graph(community_n=15, community_size_mean=10, seed=123),
                test_rate=0.1,
            ),
        )

        pop_distanced = Environment(
            name=f"{self._temp_dir.name}/A socially responsible environment, observed",
            disease=disease,
            healthcare=healthcare,
            observation_space=ObservationSpace(
                graph=Graph(
        ...
        Parallel(n_jobs=1, backend="loky")(
            delayed(run_and_replay)(pop, steps=50, plot=False, save=False)
