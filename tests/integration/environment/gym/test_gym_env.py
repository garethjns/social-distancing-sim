...
from social_distancing_sim.agent import (
...
from social_distancing_sim.environment.gym.wrappers.flatten_obs_wrapper import (
    FlattenObsWrapper,
)
from social_distancing_sim.environment.gym.wrappers.limit_obs_wrapper import (
    LimitObsWrapper,
)
...
try:
    ...
    from social_distancing_sim.agent.rl_agents.q_learning.dqn_untargeted import (
        DQNUntargeted,
    )
...
class TestGymEnv(unittest.TestCase):
    ...
    @staticmethod
    def _active_run_for(
        agent: Union[NonLearningAgentBase, LearningAgentBase], steps: int = 25
    ) -> Union[NonLearningAgentBase, LearningAgentBase]:
    ...
    def test_passive_env_fixed_seed_is_deterministic(self):
        ...
        self.assertListEqual(
            env1.unwrapped.sds_env.observation_space.graph.current_infected_nodes,
            env2.unwrapped.sds_env.observation_space.graph.current_infected_nodes,
        )
        self.assertListEqual(
            env1.unwrapped.sds_env.observation_space.graph.current_clear_nodes,
            env2.unwrapped.sds_env.observation_space.graph.current_clear_nodes,
        )
        self.assertAlmostEqual(
            env1.unwrapped.sds_env.observation_space.graph.g_.nodes[5]["immune"],
    ...
    def test_gym_env_with_dummy_agent(self):
        ...
        self.assertEqual(
            agent.env_builder.env_spec, "SDSTests-GymEnvFixedSeedFixture-v0"
        )
    ...
    def test_gym_env_with_multi_agent(self):
        ...
        agent = MultiAgent(
            ...
            agents=[
                ...
                TreatmentPolicyAgent(
                    actions_per_turn=5, start_step={"treat": 50}, end_step={"treat": 80}
                ),
                MaskingPolicyAgent(
                    actions_per_turn=10,
    ...
    def test_gym_env_with_rl_agent(self):
        # Arrange
        config_dict = RLKAgentConfigs(
            agent_name="flat_obs_dqn",
            env_spec="SDSTests-GymEnvFixedSeedFixture-v0",
            expected_obs_shape=(180,),
            env_wrappers=(partial(LimitObsWrapper, output=2), FlattenObsWrapper),
            n_actions=5,
        ).build_for_dqn_untargeted()
    ...
    @unittest.skipUnless(
        int(f"{sys.version_info.major}{sys.version_info.minor}") > 36,
        "deepcopy breaks in 3.6",
    )
