...
class TestSim(unittest.TestCase):
    ...
    def test_default_sim_run(self):
        # Arrange
        sim = Sim(env_spec='SDSTests-GymEnvDefaultFixture-v0',
                  n_steps=10,
        ...
        self.assertEqual(10, len(sim.env.sds_env.history['Turn score']))

    def test_example_sim_run(self):
        # Arrange
        sim = Sim(env_spec='SDSTests-GymEnvSpecifiedFixture-v0',
        ...
        self.assertEqual(100, len(sim.env.sds_env.history['Turn score']))

    def test_example_sim_run_with_plotting(self):
        # Arrange
        sim = Sim(env_spec='SDSTests-GymEnvSomePlottingFixture-v0',
                  save_dir=f"{self._tmp_dir.name}",
                  agent=VaccinationAgent(actions_per_turn=25, seed=123),
        ...
        self.assertEqual(12, len(sim.env.sds_env.history['Turn score']))
    ...
    def test_example_sim_run_with_extra_plotting(self):
        # Arrange
        sim = Sim(env_spec='SDSTests-GymEnvExtraPlottingFixture-v0',
                  save_dir=f"{self._tmp_dir.name}",
                  agent=VaccinationAgent(actions_per_turn=25, seed=123),
        ...
        self.assertEqual(16, len(sim.env.sds_env.history['Turn score']))
