...
RUN_TESTS = True
try:
    from reinforcement_learning_keras.agents.components.helpers.virtual_gpu import (
        VirtualGPU,
    )
...
class TestDQNUntargeted(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.gpu = VirtualGPU(gpu_memory_limit=4096, gpu_device_id=0)
    ...
    def setUp(self):
        ...
        config_dict = RLKAgentConfigs(
            agent_name=f"{self._tmp_dir.name}/flat_obs_dqn",
            env_spec=self._env,
    ...
    def test_run_agent_in_sim_render_on(self, *args):
        ...
        # Assert
        self.assertEqual(
            0, len(glob.glob(os.path.join(self._tmp_dir.name, self._png_path)))
        )
    ...
    def test_run_agent_without_sim_render_on(self, *args):
        ...
        # Assert
        self.assertEqual(
            3, len(glob.glob(os.path.join(self._tmp_dir.name, self._png_path)))
        )
