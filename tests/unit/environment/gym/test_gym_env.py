...
from tests.common.env_fixtures.env_template_random_seed_fixture import (
    EnvTemplateRandomSeedFixture,
)
...
class TestGymEnv(unittest.TestCase):
    ...
    def test_agg_state_matches_internal_env(self):
        ...
        self.assertEqual(
            obs[0][0], len(env.sds_env.observation_space.current_alive_nodes)
        )
        self.assertEqual(
            obs[0][1], len(env.sds_env.observation_space.current_clear_nodes)
        )
        self.assertEqual(
            obs[0][2], len(env.sds_env.observation_space.current_infected_nodes)
        )
        self.assertEqual(
            obs[0][3], len(env.sds_env.observation_space.current_immune_nodes)
        )
        self.assertEqual(
            obs[0][4], len(env.sds_env.observation_space.current_isolated_nodes)
        )
        self.assertEqual(
            obs[0][5], len(env.sds_env.observation_space.current_masked_nodes)
        )
    ...
    def test_graph_matrix_state_matches_internal_env(self):
        ...
        # Assert
        self.assertEqual(
            (env.sds_env.total_population, env.sds_env.total_population), obs[1].shape
        )
    ...
    def test_full_state_matches_internal_env(self):
        ...
        self.assertEqual(
            len(env.sds_env.observation_space.current_alive_nodes), obs[2][:, 0].sum()
        )
        self.assertEqual(
            len(env.sds_env.observation_space.current_clear_nodes), obs[2][:, 1].sum()
        )
        self.assertEqual(
            len(env.sds_env.observation_space.current_immune_nodes), obs[2][:, 3].sum()
        )
        self.assertEqual(
            len(env.sds_env.observation_space.current_isolated_nodes),
            obs[2][:, 4].sum(),
        )
        self.assertEqual(
            len(env.sds_env.observation_space.current_masked_nodes), obs[2][:, 5].sum()
        )
