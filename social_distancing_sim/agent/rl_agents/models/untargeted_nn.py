...
class UntargetedNN(ModelBase):
    ...
    def _model_architecture(self) -> Tuple[keras.layers.Layer, keras.layers.Layer]:
        ...
        fc1 = keras.layers.Dense(units=int(n_units), name="fc1", activation="relu")(
            state_input
        )
        fc2 = keras.layers.Dense(units=int(n_units / 2), name="fc2", activation="relu")(
            fc1
        )
        fc3 = keras.layers.Dense(units=int(n_units / 4), name="fc3", activation="relu")(
            fc2
        )
