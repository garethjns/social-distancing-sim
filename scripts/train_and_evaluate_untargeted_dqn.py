...
class EnvTemplate(TemplateBase):
    ...
    def build(cls) -> env.Environment:
        return env.Environment(
            name="agent training example",
            action_space=ActionSpace(),
            environment_plotting=EnvironmentPlotting(
                ts_fields_g2=[
                    "Vaccinate actions completed",
                    "Isolate actions completed",
                    "Reconnect actions completed",
                    "Treat actions completed",
...
if __name__ == "__main__":
    gpu = VirtualGPU(gpu_memory_limit=2048, gpu_device_id=0)
