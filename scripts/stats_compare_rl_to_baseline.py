...
from scripts.visual_compare_multi_agents import (
    DISTANCING_PARAMS,
    MASKING_PARAMS,
    TREATMENT_PARAMS,
    VACCINATION_PARAMS,
...
def plot_dists(
    multi_sims: List[sim.MultiSim], result: str = "Overall score"
) -> plt.Figure:
    ...
    for run in multi_sims:
        ...
        sns.distplot(run.results[result], hist=False, label=run.sim.agent.name)

    ax.set_title("Agent comparison", fontweight="bold")
...
if __name__ == "__main__":
    gpu = VirtualGPU(gpu_memory_limit=4096, gpu_device_id=1)
    gym.envs.register(
        id="SDS-746-v0",
    ...
    deep_q_agent = DQNUntargeted.load(
        "scripts/flat_obs_dqn_SDS-746-v0", actions_per_turn=5
    )

    agents = [
        agent.DummyAgent(name="Dummy agent"),
        agent.MultiAgent(
    ...
    multi_sims = []
    for agt in agents:
        sim_ = sim.Sim(env_spec=env_spec, tqdm_on=False, agent=agt, n_steps=steps)

        multi_sims.append(sim.MultiSim(sim_, n_jobs=1, name="rl agents", n_reps=5))
