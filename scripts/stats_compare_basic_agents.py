...
def plot_dists(
    ...
    for ax_i, (ax_name, ax) in enumerate(ax_map.items()):
        ...
        ax.set_xlim(
            [min_score - abs(min_score * 0.2), max_score + abs(max_score * 0.2)]
        )
        if ax_i != 3:
            ax.set_xlabel("")
        else:
...
class EnvTemplate(TemplateBase):
    def build(self):
        env_ = env.Environment(
            name="stats_compare_basic_agents_custom_env",
            ...
            observation_space=env.ObservationSpace(
                graph=env.Graph(community_n=15, community_size_mean=10, seed=None),
                test_rate=1,
                seed=None,
            ),
...
if __name__ == "__main__":
    ...
    for n_act, agt in np.array(np.meshgrid(n_actions, agents)).T.reshape(-1, 2):
        ...
        multi_sims.append(
            sim.MultiSim(sim_, name="basic agent comparison", n_reps=300, n_jobs=60)
        )
