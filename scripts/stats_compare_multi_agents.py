...
def plot_dists(
    ...
    ax.set_title("Policy comparison", fontweight="bold")
...
class EnvTemplate(TemplateBase):
    def build(self):
        env_ = env.Environment(
            name="stats_compare_multi_agents_custom_env",
...
def run_multi_sims():
    ...
    for agt in AGENTS:
        ...
        multi_sims.append(
            sim.MultiSim(sim_, n_jobs=30, name="policy agent comparison", n_reps=300)
        )
