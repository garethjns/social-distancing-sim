...
if __name__ == "__main__":
    ...
    pop_distanced = env.Environment(
        name="exps/A socially responsible environments",
        disease=env.Disease(name="COVID-19"),
        observation_space=env.ObservationSpace(
            graph=env.Graph(
                community_n=40,
    ...
    Parallel(n_jobs=2, backend="loky")(
        delayed(run_and_replay)(pop, steps=300, plot=False, save=save)
        for pop in [pop, pop_distanced]
    )
